...
for i = 1:length(mu)
    for j = 1:length(nsig)
        ...
        for k=1:(nt-1),
            ...
            wk=w(k);
            tspan=[t(k) t(k+1)];
            [ttmp,x_ode45]=ode45(@(t,x) vanderpol(tspan,x,curr_mu,wk),tspan,x_init);
        ...
        title("test")
        tcl = tiledlayout(1,2,'TileSpacing','compact','Padding','tight');
        nexttile;
        iix=1;
        ...
        grid
        ylabel('x_2 state');
        title(tcl,"Mu = " + string(curr_mu) + ", N_s_i_g = " + string(curr_nsig));
        ...
        for k=1:(nt-1)
            ...
            [xhatu(:,k+1),Pu(:,:,k+1),xPred,zPred,innovation]=...
                spf_vdp(xhatu(:,k),Pu(:,:,k),[],Qx,ffun,zkp1,R,hfun,t(k),t(k+1),curr_mu,curr_nsig);
        end
