...
Q_diag_vals = [0.5 0.5 0.1 0.1].^2;
else
Q_diag_vals = [0.1 0.04 0.01 0.01].^2;
end
Q=diag(Q_diag_vals);
...
P0_vals = [0.5 0.5 0.5 0.1 0.5 0.1].^2;
P0 = diag(P0_vals);
xhatu = zeros(nx,nt); xhatu(:,1)=x0;
Pu = zeros(nx,nx,nt); Pu(:,:,1)=P0;
xhatp=xhatu;
Pp=Pu;
[xhatu,Pu] = particle_filter_analysis(N_part, nx, nk, x0, P0, z, dt, H, R, N_eff_value, Zacc, Zrg);
...
function Xkp1=predict_state_carposebias_pf(Xk,U,dt)
%
%
...
function [xhatu,Pu] = particle_filter_analysis(N_part, nx, nk, x0, P0, z, dt, H, R, N_eff_value, Zacc, Zrg)
% Corrected particle filter for state:
% x = [x; y; v; theta; b_acc; b_rg]
    ...
    W(1,:) = ones(1,N_part) / N_part;

    xhatu = zeros(nx, nk);
    Pu = zeros(nx, nx, nk);

    % Initial estimate and covariance
    xhatu(:,1) = X_part(:,:,1) * W(1,:)';
    E0 = X_part(:,:,1) - xhatu(:,1);
    Pu(:,:,1) = E0 * diag(W(1,:)) * E0';

    % Process noise tuning parameters
    % These are the most important tuning knobs for x3 and x4 consistency
    sigma_v  = 0.15;    % velocity process noise
    sigma_th = 0.04;    % heading process noise
    sigma_ba = 0.002;   % accel bias random walk
    sigma_bg = 0.0015;  % gyro bias random walk

    % Measurement likelihood constants
    nz = size(R,1);
    Rinv = inv(R);
    meas_const = 1 / sqrt((2*pi)^nz * det(R));
    ...
    for k = 1:(nk-1)
        ...
        % Draw process noise
        nv  = sigma_v  * randn(1,N_part);
        nth = sigma_th * randn(1,N_part);
        nba = sigma_ba * randn(1,N_part);
        nbg = sigma_bg * randn(1,N_part);

        % Use measured IMU values, corrected by each particle's bias
        a_eff = Zacc(k) - bak;
        w_eff = Zrg(k)  - bgk;

        % Propagate particles
        xkp1  = xk  + dt * (vk .* cos(thk));
        ykp1  = yk  + dt * (vk .* sin(thk));
        vkp1  = vk  + dt * a_eff + nv;
        thkp1 = thk + dt * w_eff + nth;
        bakp1 = bak + nba;
        bgkp1 = bgk + nbg;

        X_pred = [xkp1; ykp1; vkp1; thkp1; bakp1; bgkp1];
        ...
        for ip = 1:N_part
            e = innov(:,ip);
            L(ip) = meas_const * exp(-0.5 * e' * Rinv * e);
        end
        ...
        if Neff < N_eff_value
            idx = systematic_resample(Wk);
            X_post = X_pred(:,idx);
            W_post = ones(1,N_part) / N_part;
        else
            X_post = X_pred;
            W_post = Wk;
        end

        % Store posterior particles and weights
        X_part(:,:,k+1) = X_post;
        W(k+1,:) = W_post;

        % Posterior estimate and covariance
        xhatu(:,k+1) = X_post * W_post';
        E = X_post - xhatu(:,k+1);
        Pu(:,:,k+1) = E * diag(W_post) * E';
    end
end
...
% function [xhatu,Pu] = particle_filter_analysis(N_part, nx, nk, nw, x0, P0, Qsq, z, dt, H, R, N_eff_value, Zacc, Zrg)
% X_part = zeros(nx, N_part, nk);
% X_part(:,:,1) = mvnrnd(x0, P0, N_part)';
...
%     U = [Zacc(k); Zrg(k); 0; 0];
%     X_pred = predict_state_carposebias_pf(X_prior, U + W_part, dt);
% 
%     % Update Step
%     z_current = z(:, k+1);
%     Z_hat = H*X_pred;
